...
class fpTreeNode:
    ...
    def get_display_tree_list(self,mList):
        mList.append((self.name, self.freq))
        for c in self.child.values():
            # For any children of the node, call the function recursively
            c.get_display_tree_list(mList)


"""Writes the frequent itemsets to a CSV file"""
def export_to_file(output_file_name,data):
...
def fp_tree_preprocess(doc_name, threshold):
    ...
    item_freq:Dict[str,int] = {k:v for k,v in item_freq.items() if v >= threshold}
    return data, item_freq

def fp_tree_preprocess2(item_tbl:List[List[str]],threshold):
    item_freq:Dict[str,int] = {}
    for entry in item_tbl:
        for sub_item in entry:
            if sub_item != '':
                if sub_item not in item_freq:
                    item_freq[sub_item] = 1
                else:
                    item_freq[sub_item] += 1
    item_freq:Dict[str,int] = {k:v for k,v in item_freq.items() if v >= threshold}
    res = np.array(item_tbl)
    return res,item_freq
...
"""
def create_leaf_cond_base(output_file_name, similar_item_dict, threshold):
    final_cond_base = []
    final_cond_base_key = []
    ...
    for key,value in similar_item_dict.items():
        ...
        ## Starting code for question 2B conditional tree creation
    ...
    # 去重 顺序不一样不算重
    # final_cond_base, final_cond_base_key -> unique_cond_base_set -> unique_cond_base_list
    ...
    # if output_file_name != None:
        # export_to_file(output_file_name, unique_cond_base) # Exports the asnwer to a CSV file
    return unique_cond_base_list,final_cond_base_key
...
def m(file_name,output_file_name,support):
...
def merge(FIMItemFre):
    di = {}
    fre = None
    for itemset in FIMItemFre:
        tmpItemList = []
        for entry in itemset:
            tmpItemList.append(entry[0])
            fre = entry[1]
        tup = tuple(tmpItemList)
        if tup not in di:
            di[tup] = 0
        di[tup] += fre
    result = []
    for key in di:
        tmpEntry = {}
        tmpEntry['usage_seq'] = list(key)
        tmpEntry['frequency'] = di[key]
        result.append(tmpEntry)
    return result


def runFIM(usage_tbl,output_file_name,support):
    # Function calling in the main program to implement FP Growth algorithm
    dataset, freq_items = fp_tree_preprocess2(usage_tbl, support)
    fptree_root, header_table = fp_tree_reorder(dataset, freq_items)
    # List[Tuple[str]]
    FIMItem,FIMItemFre = create_leaf_cond_base(output_file_name, header_table,support)
    # Dict[Tuple[str],int]
    FIMItemFre2 = merge(FIMItemFre)
    return FIMItem,FIMItemFre2

# path = '/Users/huangkaifeng/Library/Mobile Documents/com~apple~CloudDocs/Workspace/CIAtool-Scripts/data.nosync/D_MiningFrequentItems/groceries.csv'
# m(path,output_file_name,support)
def test():
    a = []
    b1 = ['A','B','C','E','F','O']
    b2 = ['A','C','G']
